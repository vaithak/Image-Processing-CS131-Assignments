...
def energy_function(image):
    ...
    ### YOUR CODE HERE
    out = abs(np.gradient(gray_image, axis=0))
    out += abs(np.gradient(gray_image, axis=1))
    ### END YOUR CODE
...
#             paths[i,j] = np.argmin(cost[i-1, max(j-1,0):min(j+2, cost.shape[1])])
#             if(j != 0): 
#                 paths[i,j]-=1
#             cost[i,j] = energy[i,j] + cost[i-1, j+paths[i,j]]
    for i in range(1, H):
        ...
        cost[i] = energy[i] + M[paths[i], np.arange(M.shape[1])]
        paths[i] -= 1
...
def enlarge(image, size, axis=1, efunc=energy_function, cfunc=compute_cost, dfunc=duplicate_seam, bfunc=backtrack_seam, rfunc=remove_seam):
    ...
    for i in range(size - W):
        ...
        out = duplicate_seam(out, curr_seam)
        seams = duplicate_seam(seams, curr_seam)
    ### END YOUR CODE
