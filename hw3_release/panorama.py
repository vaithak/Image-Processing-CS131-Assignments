...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    for i in range(dists.shape[0]):
        curr_dists = np.argsort(dists[i])[0:2]
        ratio = dists[i, curr_dists[0]]/dists[i, curr_dists[1]]
        if(ratio < threshold):
            matches.append([i, curr_dists[0]])
...
def fit_affine_matrix(p1, p2, pad=True):
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for i in range(n_iters):
        ...
        curr_H = fit_affine_matrix(matched1[idx], matched2[idx], False)

        curr_inliers = np.linalg.norm(matched2.dot(curr_H) - matched1, axis=1) ** 2 < threshold
        curr_n_inliers = np.sum(curr_inliers)
        if curr_n_inliers > n_inliers:
            max_inliers = curr_inliers.copy()
            n_inliers = curr_n_inliers
            
    H = fit_affine_matrix(matched1[max_inliers], matched2[max_inliers], False)
    ### END YOUR CODE
...
def hog_descriptor(patch, pixels_per_cell=(8,8)):
    ...
    for i in range(rows):
        for j in range(cols):
            cells[i][j] = np.histogram(theta_cells[i][j].flatten(), bins=n_bins, weights=G_cells[i][j].flatten())[0]
